...
off_t dir_walk(char *, char *);				//function to traverse a directory and return its size takes argument filename and path
...
off_t dir_walk(char * dir_name, char *path)					//takes current directory name and its path as arguments
{
	...
	{	
		...
		else
		{
			char temp[1000];
			strcpy(temp, path);			
			dir_walk(dir_entry->d_name, strcat(strcat(temp, "/"), dir_entry->d_name));		
		}
		close(fd);
