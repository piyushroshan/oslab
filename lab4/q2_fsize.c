...
off_t fsize(char *, char *);				//function to traverse a directory and return its size takes argument filename and path
...
off_t fsize(char * dir_name, char *path)					//takes current directory name and its path as arguments
{
	...
	dir_size+=fileStat.st_blocks*512;
	...
	{	
		...
		if(!S_ISDIR(fileStat.st_mode))				//if current entry is not a directory in itself, simply add its size
		{
			...
			char temp[1000];
			strcpy(temp, path);	
			prnsize((fileStat.st_blocks*512)/1024);
		...
		else
		{
			char temp[1000];
			strcpy(temp, path);			
			dir_size+=fsize(dir_entry->d_name, strcat(strcat(temp, "/"), dir_entry->d_name));		/*if the directory entry is a directory by itself
																									then get its size by recursively calling the
																									fsize fn on it. to get path of the directory,
